...
public class EhcacheDemo {
    ...
    private void withoutCache() {
        ...
        IntStream.range(1, 10).forEach(
                val -> logger.info("withoutCache value: {}", SlowDataSrc.getValue(val)));
    ...
    private void withCache() {
        ...
        logger.info("second getting...");
        IntStream.range(1, 10).map(i -> 10 - i).forEach(
                val -> logger.info("value: {}", getValue(val)));
